...
TESTTYPE="skip-on-rhel-9 payload uefi bootc reboot storage gh1574"
...
copy_interesting_files_from_system() {
    ...
    # Find root device by filesystem label from reqpart_root.ks
    ...
    if [ -z "${root_device}" ]; then
        echo "Couldn't find root filesystem with label rootfs"
        return 1
    fi

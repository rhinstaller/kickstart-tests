...
# Check that /etc/resolv.conf file was generated by NetworkManager
function check_resolv_conf_is_by_nm() {
    egrep -q '^# Generated by NetworkManager'  /etc/resolv.conf
    if [[ $? -ne 0 ]]; then
        echo "*** Failed check: /etc/resolv.conf is generated by NetworkManager" >> /root/RESULT
    fi
}

# Check that /etc/resolv.conf file was generated by systemd-resolved
function check_resolv_conf_is_by_resolved() {
    local real_path=$(realpath /etc/resolv.conf)
    local expected_path="/run/systemd/resolve/stub-resolv.conf"
    if [[ ${real_path} != ${expected_path} ]]; then
        echo "*** Failed check: real path of /etc/resolv.conf: ${real_path} expected path: ${expected_path}" >> /root/RESULT
    fi
}
